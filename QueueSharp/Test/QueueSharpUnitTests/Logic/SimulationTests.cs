...
public class SimulationTests
{
    ...
    public void CiwComparison_SingleNode_SameArrivalDistribution()
    {
        ...
        Cohort[] cohorts = CohortFactory.GetSingleNode("Node01",
                serverCount: 3,
                new EmpiricalDuration(empiracalInterarrivalTimes.Select(x => (int)Math.Round(x, 0)), false),
    ...
    public void CiwComparison_SingleNode()
    {
        Cohort[] cohorts = CohortFactory.GetSingleNode("Node01",
        serverCount: 3,
                arrivalDistribution: new ExponentialDistribution(0.002),
                serviceDistribution: new ExponentialDistribution(0.001),
    ...
    public void CiwComparison_RestrictedNetworks()
    {
        ...
        IRouting routing = new RandomRouteSelection(arcs, queueFullBehaviorByNode.ToFrozenDictionary());
        IServerSelector serverSelector = new FirstServerSelector();
        ...
        Dictionary<Node, NodeProperties> propertiesByNode = new()
        {
            {
                nodes[0],
                new NodeProperties(ArrivalDistributionSelector: DistributionFactory.CreateConstant(start: 0, end: simulationDuration * 2, duration: 330, arrivalFraction: 1),
                    ServiceDurationSelector: DistributionFactory.CreateConstant(start: 0, end: simulationDuration * 2, duration: 350),
                    ServerSelector: serverSelector)
            },
            {
                nodes[1],
                new NodeProperties(ArrivalDistributionSelector: DurationDistributionSelector.None,
                    ServiceDurationSelector: DistributionFactory.CreateConstant(start: 0, end: simulationDuration * 2, duration: 400),
                    ServerSelector: serverSelector)
            },
            {
                nodes[2],
                new NodeProperties(ArrivalDistributionSelector: DurationDistributionSelector.None,
                    ServiceDurationSelector: DistributionFactory.CreateConstant(start: 0, end: simulationDuration * 2, duration: 450),
                    ServerSelector: serverSelector)
            },
        };
